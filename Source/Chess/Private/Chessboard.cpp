...
void AChessboard::SpawnSinglePiece(ATile* CurrentTile, const EPieceTeam Team, const EPieceType Type)
{
	...
	AChessPiece* Piece = GetWorld()->SpawnActor<AChessPiece>(PieceClass[Type], SpawnLocation, SpawnRotation);
	...
	case EPieceTeam::WHITE:
		...
		{
			GameMode->Kings[EPieceTeam::WHITE] = Cast<AChessKing>(Piece);
		}
		break;
	case EPieceTeam::BLACK:
		...
		if (Type == EPieceType::KING)
		{
			GameMode->Kings[EPieceTeam::BLACK] = Cast<AChessKing>(Piece);
		}
		break;
	default:
		break;
